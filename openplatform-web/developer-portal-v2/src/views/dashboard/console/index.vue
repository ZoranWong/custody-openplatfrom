      ...
      <ElCol :sm="24" :md="24" :lg="12">
        <div class="art-card p-5 mb-5">
          ...
          <ElTable v-if="recentErrors.length > 0" :data="recentErrors" size="small" stripe>
            <ElTableColumn prop="apiName" :label="$t('developer.billing.endpointBreakdown.path')" min-width="160" />
            <ElTableColumn prop="responseStatus" :label="$t('developer.billing.payment.status')" width="90">
              <template #default="{ row }">
                <ElTag type="danger" size="small">{{ row.responseStatus }}</ElTag>
              </template>
            </ElTableColumn>
            <ElTableColumn prop="createdAt" :label="$t('developer.billing.payment.submittedAt')" width="110">
    ...
    <ElRow :gutter="20">
      <ElCol :span="24">
        <div class="art-card p-5 mb-5">
          ...
          <EndpointBreakdown v-if="usageStats?.endpoint_breakdown?.length" :endpoints="usageStats.endpoint_breakdown" :loading="loading" />
          <ElEmpty v-else :image-size="60" />
        </div>
      </ElCol>
    </ElRow>
...
const subDesc = computed(() => {
  if (!subscription.value) return t('developer.dashboard.stats.freePlan')
  const name = subscription.value.package?.name || ''
  const end = formatDate(subscription.value.endDate, 'date')
  return `${name} · ${end}`
})

const statCards = computed(() => [
  {
    key: 'apps', icon: 'ri:apps-line', iconStyle: 'bg-blue-500',
    title: t('menus.developer.applications'),
    count: appCount.value as number,
    description: t('developer.dashboard.stats.applications')
  },
  {
    key: 'todayCalls', icon: 'ri:bar-chart-line', iconStyle: 'bg-green-500',
    title: t('developer.usageStatistics.todayCalls'),
    count: usageStats.value?.today_calls ?? 0,
    description: `${t('developer.dashboard.stats.dailyQuota')}: ${usageStats.value?.daily_limit ?? 0}`,
    separator: ','
  },
  {
    key: 'remainingCalls', icon: 'ri:contrast-drop-2-line', iconStyle: 'bg-orange-500',
    title: t('developer.usageStatistics.remainingCalls'),
    count: Math.max(0, (usageStats.value?.daily_limit ?? 0) - (usageStats.value?.today_calls ?? 0)),
    description: `${usageStats.value?.today_calls ?? 0}/${usageStats.value?.daily_limit ?? 0}`,
    separator: ','
  },
  {
    key: 'successRate', icon: 'ri:check-double-line', iconStyle: 'bg-amber-500',
    title: t('developer.applications.apiUsage.successRate'),
    count: usageStats.value?.success_rate ?? 0,
    description: t('developer.dashboard.stats.last30Days'),
    decimals: 1
  },
  {
    key: 'avgResponse', icon: 'ri:timer-line', iconStyle: 'bg-purple-500',
    title: t('developer.usageStatistics.avgResponse'),
    count: usageStats.value?.avg_response_time_ms ?? 0,
    description: 'ms'
  },
  {
    key: 'subscription', icon: 'ri:vip-crown-line', iconStyle: 'bg-indigo-500',
...
const trendData = computed(() => {
  ...
  return [
    { name: t('developer.usageStatistics.apiCalls'), data: usageStats.value.daily_breakdown.map((d: any) => d.calls ?? 0) },
    { name: t('developer.billing.endpointBreakdown.successCount'), data: usageStats.value.daily_breakdown.map((d: any) => d.success_count ?? 0) }
  ]
})
