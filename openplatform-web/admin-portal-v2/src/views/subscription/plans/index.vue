  ...
  <div class="subscription-plans-page art-full-height">
    <div class="flex items-center justify-between mb-4">
      ...
      <ElSpace>
        ...
        <ElButton @click="showHistory = !showHistory">
          {{ showHistory ? '查看可用套餐' : '查看历史记录' }}
        </ElButton>
      </ElSpace>
    </div>

    <!-- Active Packages - Card Layout -->
    <ElRow v-if="!showHistory" :gutter="20">
      <ElCol v-for="pkg in activePackages" :key="pkg.id" :sm="24" :md="12" :lg="6">
        <ElCard class="package-card cursor-pointer" :class="packageTypeClass(pkg.packageCode)" shadow="hover" @click="handleCardClick(pkg)">
          <template #header>
          ...
          <ElDivider />
          <div class="features">
            <p v-for="(f, i) in parseFeatures(pkg.features)" :key="i" class="text-sm text-gray-600 mb-1">
      ...
      <ElCol v-for="code in missingPackageTypes" :key="code" :sm="24" :md="12" :lg="6">
        <ElCard class="package-card package-card-empty" shadow="hover">
          <div class="flex flex-col items-center justify-center h-full py-8 text-gray-400">
            ...
            <ElButton type="primary" size="small" @click.stop="handleAddForType(code)">新增套餐</ElButton>
          </div>
        </ElCard>
      </ElCol>
    </ElRow>
    ...
    <div v-else>
      <div class="mb-4">
        <ElSelect v-model="historyFilter" placeholder="筛选套餐类型" clearable style="width: 200px" @change="loadHistory">
    ...
    >
      <ElForm ref="formRef" :model="formData" :rules="formRules" label-width="120px">
        <ElFormItem label="套餐类型" prop="packageCode">
          <ElSelect v-model="formData.packageCode" :disabled="!!editingPackage" placeholder="选择套餐类型">
            <ElOption label="体验版 (TRIAL)" value="TRIAL" />
            <ElOption label="基础版 (BASIC)" value="BASIC" />
            <ElOption label="中小企业版 (PROFESSIONAL)" value="PROFESSIONAL" />
            <ElOption label="金融服务大型企业版 (ENTERPRISE)" value="ENTERPRISE" />
...
const dialogTitle = computed(() => (editingPackage.value ? '编辑套餐（将创建新版本）' : '新增套餐'))
...
// Watch for history tab to load data
